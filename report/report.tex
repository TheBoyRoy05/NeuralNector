...
\icmlkeywords{Generative Adversarial Networks, Flowers 102, MNIST, DCGAN, PyTorch Lightning}
...
\vskip 0.1in
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{images/GAN_architecture}}
...
The original minimax objective \cite{GANs} is defined as follows:
\begin{equation}
...
We mitigated these effects with practices such as batch normalization in the deeper networks, label smoothing, asymmetric learning rates for $G$ and $D$, and selective updates that skip or repeat discriminator steps when its loss indicates it is overpowering the generator; details appear in \cref{sec:methods}.
...
\vskip 0.1in
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{images/flower_collapse}}
...
\vskip 0.1in
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{images/epochs}}
...
The generator maps Gaussian noise of dimension $z\in\mathbb{R}^{100}$ through a linear layer to a $7\times 7\times 64$ tensor, applies two transposed convolutions to reach spatial size $28\times 28$, and a final convolution with \texttt{tanh}. ReLU activations are used in the generator, and ReLU with pooling in the discriminator.
...
\subsection{Selecting fakes for \underline{\href{https://neuralnector.com}{NeuralNector}}}
...
We report two complementary evaluations: (i) a \emph{metrics-driven} Flowers~102 study reporting Fr\'echet Inception Distance (FID) \cite{Heusel2017GANs}, Kernel Inception Distance (KID), and Inception Score (IS) \cite{ImprovedGANs} via the \texttt{torch-fidelity} library on held-out test reals, and (ii) a lightweight MNIST ablation focused on optimization behavior (BCE losses only). The Flowers protocol is implemented in \texttt{GAN\_experiments.ipynb}: five DCGAN runs of $150$ epochs at batch size $64$ ($\approx 4.8\times 10^3$ optimizer steps per run), targeting roughly one hour of GPU time on a typical Colab instance, which is still far shorter than the multi-thousand-epoch production training in \texttt{GAN\_pytorch.ipynb}. Hyperparameters follow \cref{tab:hparams} unless an ablation overrides them. All Flowers metrics use $N=2048$ generated samples matched against the first $N$ images of the official \emph{test} split (deterministic resize/center crop to $64\times 64$, no augmentation), so training never sees the reference set. FID/KID/IS rely on Inception-v3 features and are most reliable as \emph{relative} comparisons across rows in \cref{tab:flowers-fid}; at $64\times 64$ and with a single seed they should not be read as absolute benchmarks.

\subsection{Flowers: FID, KID, and Inception score}

\cref{tab:flowers-fid} reports five runs from a single Colab execution (NVIDIA A100-40GB; $\approx 57$ minutes wall time summing training and metric computation). Configurations are \texttt{FL1\_baseline} (default stabilizers and augmentation), \texttt{FL2\_no\_aug}, \texttt{FL3\_always\_train\_D} (no adaptive skipping of $D$), \texttt{FL4\_symmetric\_lr} ($\mathrm{lr}_G=\mathrm{lr}_D$), and \texttt{FL5\_no\_label\_smooth}. We log BCE-style $L_G$, $L_D$, and batch $d_{\mathrm{acc}}$ (fraction of reals above $0.5$ and fakes below $0.5$).

By FID and KID, \texttt{FL4\_symmetric\_lr} is strongest on this seed (FID $211$, KID $0.180$), with the highest IS as well---noting that IS is less directly aligned with natural flowers than with ImageNet-like class structure. \texttt{FL2\_no\_aug} is weakest (FID $308$, IS $1.75$), supporting the use of augmentation for this small-data regime. \texttt{FL5\_no\_label\_smooth} improves over \texttt{FL1} on FID/KID despite similar late-epoch losses, while \texttt{FL3\_always\_train\_D} lands between \texttt{FL1} and \texttt{FL2} on FID without the catastrophic MNIST loss split seen when $D$ is always updated on digits (\cref{tab:mnist-ablations}); Flowers optimization appears less brittle to that ablation on this budget. Overall, metrics disagree with a naive reading of BCE alone (e.g., \texttt{FL4} has higher $L_G$ than \texttt{FL1} but better FID), underscoring the value of distribution metrics alongside losses.

\begin{table*}[t]
\caption{Flowers~102 generative metrics ($150$ epochs, batch $64$, $N=2048$ fakes vs.\ test reals, one seed). Lower FID/KID is better. $d_{\mathrm{acc}}$: final epoch; $\bar{d}_{\mathrm{acc}}^{(3)}$: mean over last three epochs. Source: \texttt{GAN\_experiments.ipynb} / \texttt{EXPERIMENTS\_JSON}.}
\label{tab:flowers-fid}
\vskip 0.1in
\begin{center}
\begin{small}
\begin{sc}
...
\toprule
Run & FID$\downarrow$ & KID & IS & $L_G$ & $L_D$ & $d_{\mathrm{acc}}$ & $\bar{d}_{\mathrm{acc}}^{(3)}$ \\
\midrule
\texttt{FL1\_baseline} & $279.1$ & $0.272$ & $2.23$ & $0.778$ & $0.704$ & $0.505$ & $0.511$ \\
\texttt{FL2\_no\_aug} & $307.8$ & $0.331$ & $1.75$ & $1.079$ & $0.643$ & $0.662$ & $0.672$ \\
\texttt{FL3\_always\_train\_D} & $302.2$ & $0.301$ & $2.08$ & $0.753$ & $0.705$ & $0.499$ & $0.510$ \\
\texttt{FL4\_symmetric\_lr} & $211.0$ & $0.180$ & $3.43$ & $1.180$ & $0.643$ & $0.660$ & $0.667$ \\
\texttt{FL5\_no\_label\_smooth} & $239.9$ & $0.214$ & $2.34$ & $0.770$ & $0.706$ & $0.489$ & $0.492$ \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
...
For a qualitative view of the flower generator used in deployment after long training, see \cref{fig:final-flowers-grid} (trained and evaluated as described in \cref{sec:conclusion}).

\begin{table}[t]
\caption{MNIST ablations ($12$ epochs, batch $128$). $L_G,L_D$: losses at the final epoch; $\bar{L}^{(3)}$: mean over the last three epochs.}
...
\begin{tabular}{lcccc}
\toprule
Run & $L_G$ & $L_D$ & $\bar{L}_G^{(3)}$ & $\bar{L}_D^{(3)}$ \\
\midrule
\texttt{M1\_baseline} & $0.697$ & $0.581$ & $0.689$ & $0.584$ \\
\texttt{M2\_no\_label\_smooth} & $0.612$ & $0.578$ & $0.608$ & $0.580$ \\
\texttt{M3\_symmetric\_lr} & $0.594$ & $0.585$ & $0.592$ & $0.586$ \\
\texttt{M4\_always\_train\_D} & $2.258$ & $0.342$ & $2.239$ & $0.343$ \\
\texttt{M5\_latent\_50} & $0.683$ & $0.585$ & $0.684$ & $0.585$ \\
\texttt{M6\_beta1\_0.9} & $0.681$ & $0.585$ & $0.682$ & $0.585$ \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
...
The Flowers metrics use a \emph{single} random seed, $N=2048$ samples per side, and $64\times 64$ inputs, so FID/KID/IS have non-trivial variance; we use them for \emph{relative} ranking across ablations in \cref{tab:flowers-fid}. The MNIST block remains loss-only. Neither study replaces a full multi-seed sweep or human evaluation at scale.

\cref{tab:flowers-fid} fixes the base learning rate at $2\times 10^{-4}$ (DCGAN-style default in \cref{tab:hparams}) and does not include a full grid over learning rate or optimizer choice (e.g., Adam vs.\ SGD) at matched compute; \texttt{FL4\_symmetric\_lr} instead varies the \emph{relative} rates $\mathrm{lr}_G$ and $\mathrm{lr}_D$. Repeating the FID protocol across seeds and over $\{10^{-4},\,2\times 10^{-4},\,4\times 10^{-4}\}$ would better match the breadth of tuning but would be much more compute-intensive.
...
\vskip 0.1in
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{images/flowers_3000.png}}
\caption{Qualitative samples from the production flower generator (baseline recipe, $3000$ epochs).}
\label{fig:final-flowers-grid}
\end{center}
\vskip -0.4in
\end{figure}

We implemented DCGAN-style adversarial training for MNIST and Oxford Flowers~102, scaling architecture and data pipeline with dataset difficulty, and documented practical failure modes (discriminator dominance, mode collapse, vanishing gradients) familiar from the GAN literature. \cref{tab:flowers-fid} shows that the baseline was around the median for many metrics, while symmetric learning rates achieved the best FID/KID over $150$ epochs, and removing augmentation hurt all reported metrics.

The deployed game uses the \emph{asymmetric} baseline in \cref{tab:hparams}, trained $3000$ epochs ($\approx 4$ GPU-hours on an NVIDIA A100-40GB, \texttt{GAN\_pytorch.ipynb}). With the same $N=2048$ FID protocol as \cref{tab:flowers-fid} (generated vs.\ held-out test reals), that checkpoint reaches FID $50.3$, far below the $150$-epoch runs (best: $\approx 211$ for \texttt{FL4\_symmetric\_lr}), so most perceptual gain comes from long optimization. \cref{fig:final-flowers-grid} shows typical samples. We still prefer this recipe over the short-run winner because it was validated at long horizon and reflects the original goal of slowing $D$ early. The generator powers \underline{\href{https://neuralnector.com}{NeuralNector}}, where human players discriminate real from synthetic images ranked in part by discriminator scores.

Future work could add multi-seed FID confidence intervals, sweep learning rates and thresholds more densely, and explore alternative objectives or regularizers while preserving the game's latency and data-pipeline constraints.

\section{Bonus}
\label{sec:bonus}

This project goes beyond training a standard benchmark in isolation in three ways: 

\begin{enumerate}
\item \textbf{Application:} a deployed web game (\href{https://neuralnector.com}{NeuralNector}) where humans act as the discriminator on mixed real and GAN-generated flowers, which requires a data and inference pipeline beyond notebook training.
\item \textbf{Engineering and evaluation:} PyTorch Lightning training with manual optimization, adaptive $D$/$G$ scheduling, a Colab reproducible FID/KID/IS study on held-out test images, and reporting of long-run production quality (FID $50.3$ after $\approx 4$ GPU-hours) versus short ablations.
\item \textbf{Empirical narrative:} documented mode collapse and stabilizers (label smoothing, asymmetric learning rates, batch normalization, selective updates) tied to both qualitative figures and quantitative tables.
\end{enumerate}

These elements reflect substantial integration effort and a readable account of practical GAN training rather than a single off-the-shelf run.
