...
For a qualitative view of the flower generator used in deployment after long training, see \cref{fig:final-flowers-grid}.
...
We implemented DCGAN-style adversarial training for MNIST and Oxford Flowers~102, scaling architecture and data pipeline with dataset difficulty, and documented practical failure modes (discriminator dominance, mode collapse) familiar from the GAN literature. MNIST ablations show that adaptive balancing of $G$ and $D$ updates can matter even when BCE losses look similar (\cref{tab:mnist-ablations}). On Flowers, \cref{tab:flowers-fid} shows that symmetric learning rates achieved the best FID/KID in our short Colab study, while removing augmentation hurt all reported metrics; we retain the default asymmetric recipe for deployment but treat these results as motivation for future hyperparameter search (\texttt{GAN\_experiments.ipynb}). The trained flower generator is deployed in \underline{\href{https://neuralnector.com}{NeuralNector}}, where human players discriminate real from synthetic images selected in part by discriminator scores.

The live game uses the current \emph{baseline} model (\cref{tab:hparams}) trained for $3000$ epochs over approximately $4$ GPU-hours in \texttt{GAN\_pytorch.ipynb}. With the same $N=2048$ FID setup as \cref{tab:flowers-fid} (generated vs.\ held-out test reals), that checkpoint achieves FID $50.3$, far below the $150$-epoch ablations (best short run: FID $\approx 211$ for \texttt{FL4\_symmetric\_lr}). The gap underscores how much of the perceptual gain comes from extended optimization rather than from the controlled one-hour experiment alone; \cref{fig:final-flowers-grid} illustrates the resulting samples.
